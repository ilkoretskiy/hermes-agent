...
def reconcile_profile_gateways(
    *, hermes_home: Path, scandir: Path, dry_run: bool = False,
    ...
    Multi-container knobs (several containers sharing one ``$HERMES_HOME`` volume):

    * ``HERMES_GATEWAY_RECONCILE_DISABLED`` (1/true/yes) — skip the whole pass; register
      nothing, not even the ``default`` slot. For containers whose only job is a single CMD
      gateway or a dashboard: their s6-log loggers would otherwise collide with the
      supervising container's on the same lockfiles ("Resource busy" spam).
    * ``HERMES_GATEWAY_MANAGED_PROFILES`` — comma-separated allowlist of named profiles this
      container may register. Named profiles outside the list are skipped (the ``default``
      slot is always registered), so it never grabs a profile another container runs as CMD.
    """
    if _env_flag("HERMES_GATEWAY_RECONCILE_DISABLED"):
        log.info("HERMES_GATEWAY_RECONCILE_DISABLED set — skipping profile gateway "
                 "reconciliation (this container supervises no profiles)")
        return []
    managed = _parse_managed_profiles(os.environ.get("HERMES_GATEWAY_MANAGED_PROFILES", ""))
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            ...
            if entry.name == "default":
                ...
                continue
            # Multi-container allowlist: a profile another container runs as CMD would be
            # double-started and fight for its Telegram token-lock / s6 log lockfiles.
