...
# ---------- Mnemosyne memory provider (baked: pip into .venv is lost on recreate) ----------
# Must run BEFORE the /opt/hermes immutability lockdown below — mnemosyne
# installs into /opt/hermes/.venv, which `chmod -R a-w` makes read-only.
...
#
# /opt/hermes/bin is prepended ahead of the venv so the privilege-drop shim
# wins PATH resolution; its last act is to exec the venv binary by absolute
# path, so the ordering is transparent. PATH order: shim, venv, user-local,
# persistent tools, then system.
ENV PATH="/opt/hermes/bin:/opt/hermes/.venv/bin:/opt/data/.local/bin:/opt/tools/bin:${PATH}"
