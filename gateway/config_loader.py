...
_RETIRED_SLACK_EXTRA_KEYS = frozenset({
    "markdown_blocks_default",
    "markdown_blocks_channels",
})


def retire_legacy_slack_config(platforms_data: dict) -> None:
    """Keep removed Slack Markdown shims inert after generic adapter-key promotion.

    ``merge_platform_sections`` and ``_bridged_keys`` intentionally preserve unknown adapter
    settings, including keys in explicit ``extra`` blocks.  These two keys are different: the
    maintained fork retired them in favor of ``platforms.slack.extra.markdown_blocks``.  Remove
    them from both locations before ``PlatformConfig.from_dict`` can promote them back into
    active adapter configuration.
    """
    slack_data = platforms_data.get(Platform.SLACK.value)
    if not isinstance(slack_data, dict):
        return
    extra = slack_data.get("extra")
    for key in _RETIRED_SLACK_EXTRA_KEYS:
        slack_data.pop(key, None)
...
def load_yaml_layer(home: Path, gw_data: dict) -> None:
    ...
    retire_legacy_slack_config(platforms_data)
