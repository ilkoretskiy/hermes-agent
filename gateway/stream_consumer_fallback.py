...
class StreamFallbackMixin:
    ...
    def _fallback_len_budget(self) -> "tuple[Callable[[str], int], int]":
        """(len_fn, raw limit) for fallback chunking, including rich overflow capacity."""
        raw_limit = self._raw_message_limit()
        _len_fn: "Callable[[str], int]" = len
        if isinstance(self.adapter, _BasePlatformAdapter):
            _len_fn = self.adapter.message_len_fn
            try:  # per-chat unit (relay adapter fronting N platforms)
    ...
    def _raw_message_limit(self) -> int:
        ...
        if isinstance(self.adapter, _BasePlatformAdapter):
            try:
                resolved_base = self.adapter.max_message_length_for_chat(self.chat_id)
                if isinstance(resolved_base, int):
                    base = resolved_base
            except Exception as e:
