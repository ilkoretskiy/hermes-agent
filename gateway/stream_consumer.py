...
class GatewayStreamConsumer:
                    ...
                    if (
                        ...
                        if got_done:
                            ...
                            self._final_response_sent = (
                                chunks_delivered and tail_delivered
                            )
    ...
    async def _abandon_native_stream(self) -> None:
        ...
        """
        # A failed native frame disables further draft sends, but the adapter
        # may still retain an ambiguously accepted stream that cancellation or
        # stale-turn cleanup must seal. Gate on this run's draft lifecycle,
        # not the transport flag that failure intentionally turns off.
        if self._draft_id is None:
            return
