...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list the default model and configured aliases."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        def add_model(model_id: str, root: str, parent: Optional[str]) -> None:
            if model_id in seen:
                return
            seen.add(model_id)
            models.append({
        ...
        # Legacy fork config: platforms.api_server.extra.models advertises the
        # explicitly configured OpenAI-compatible aliases. Preserve the old
        # aliases-only discovery shape when only this map is configured.
        if self._models_map:
            for alias, spec in sorted(self._models_map.items()):
                add_model(alias, alias, None)
        if not self._models_map or self._model_routes:
            add_model(self._model_name, self._model_name, None)
        ...
        for alias, route_cfg in self._model_routes.items():
            add_model(alias, route_cfg.get("model", alias), self._model_name)
    ...
    def _resolve_model_override(self, requested: Any) -> Optional[Dict[str, Any]]:
        ...
        if self._resolve_route(requested) is not None:
            return None
