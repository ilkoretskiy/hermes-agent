...
class SlackAdapter(BasePlatformAdapter):
    ...
    # A Slack native stream creates the visible history message.  Turn-final
    # delivery must seal/update that same message rather than post a duplicate.
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._team_bot_ids: Dict[str, str] = {}
        ...
        # Native streams are owned by workspace/chat/turn, not just chat_id.
        # Values include chat_id, team_id, turn_id, thread_anchor, ts,
        # draft_id, sent, started, and optional stopped/append_uncertain flags.
        self._active_streams: Dict[str, Dict[str, Any]] = {}
        # Coalesce concurrent first frames for one stream key.
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._team_bot_ids, self._team_bot_names = {}, {}
            self._app = AsyncApp(
    ...
    async def disconnect(self) -> None:
        ...
        self._team_clients, self._team_bot_user_ids = {}, {}
        self._team_bot_ids, self._team_bot_names = {}, {}
        self._channel_team, self._channel_teams, self._dm_conversation_cache = {}, {}, {}
        self._release_platform_lock()
    ...
    def _workspace_thread_marker(team_id: str, channel_id: str, thread_ts: str) -> Any:
        """Scope ephemeral thread evidence by workspace, channel, and thread."""
        if team_id:
    ...
    def _resolve_workspace_team_scope(
        ...
        """Resolve one workspace without borrowing another workspace's state."""
    ...
    def _admit_workspace_team_scope(
        ...
        if resolved_team_id is None:
            return None
        if resolved_team_id and self._get_client(
            channel_id, team_id=resolved_team_id
        ) is None:
            return None
        return resolved_team_id

    def _bot_user_id_for_team_scope(self, team_id: str) -> Optional[str]:
        """Resolve our bot user ID without crossing workspace scope."""
        team_bot_user_ids = getattr(self, "_team_bot_user_ids", {})
        if team_id:
            scoped = team_bot_user_ids.get(team_id)
            if scoped:
                return scoped
    ...
    def _bot_id_for_team_scope(self, team_id: str) -> Optional[str]:
        """Resolve our Slack bot_id without crossing workspace scope."""
    ...
    def _get_client(self, chat_id: str, team_id: Optional[str] = None) -> Any:
        """Return the workspace-specific WebClient without cross-team fallback."""
            ...
            if sent_ts:
                ...
                if thread_ts:
                    ...
                    self._mark_slack_thread_active(
                        team_id=team_id, channel_id=chat_id, thread_ts=str(thread_ts)
                    )
    ...
    def _record_uploaded_file_thread(
        ...
        team_id = self._metadata_team_id(metadata)
        self._bot_message_ts.add(self._workspace_message_marker(team_id, thread_ts))
        if self._slack_thread_routing_enabled():
            self._mark_slack_thread_active(
                team_id=team_id, channel_id=chat_id, thread_ts=str(thread_ts)
            )
        self._trim_bot_message_timestamps()
    ...
    def _remember_processed_message_ts(self, ts: str, team_id: str = "") -> None:
        ...
        unfurls) and after construction (refreshes LRU recency). Workspace-scoped and bounded."""
        if not ts:
            return
    ...
    async def _handle_assistant_thread_lifecycle_event(
        ...
        if team_id is None:
            logger.debug(
                "[Slack] Dropping Assistant lifecycle event with unresolved workspace client"
            )
            return
    ...
    async def _handle_app_context_changed(self, event: dict, body: Optional[dict] = None) -> None:
        ...
        team_id = self._admit_workspace_team_scope(
            fields["context_channel_id"], fields["team_id"]
        )
        if team_id is None:
            logger.debug(
                "[Slack] Dropping app_context_changed with unresolved workspace client"
            )
            return
        fields["team_id"] = team_id
        self._cache_agent_view_context(fields)
    ...
    async def _handle_app_home_opened(self, event: dict, body: Optional[dict] = None) -> None:
        ...
        fields = self._agent_view_event_fields(event, body)
        team_id = self._admit_workspace_team_scope(channel_id, fields["team_id"])
        if team_id is None:
            logger.debug(
                "[Slack] Dropping app_home_opened with unresolved workspace client"
            )
            return
        fields["team_id"] = team_id
    ...
    async def _handle_slack_reaction(
        self, event: dict, body: Optional[dict] = None, removed: bool = False
    ) -> None:
        ...
        if not bot_uid:
            logger.debug("[Slack] Dropping reaction with unresolved bot identity")
            return
    ...
    def _normalize_changed_message(self, event: dict) -> Optional[dict]:
        ...
        None if malformed. The edit's own ts rides along as ``_slack_changed_event_ts`` for dedup;
        processed-message suppression happens after workspace admission in ``_prefilter_inbound``."""
    ...
    async def _handle_slack_message(self, event: dict, payload: Optional[dict] = None) -> None:
        ...
        if not isinstance(_claim_event, dict):
            _claim_event = event or {}
        _ts = str(_claim_event.get("ts") or "")
        _channel_id = str(_claim_event.get("channel") or (event or {}).get("channel") or "")
        _team_id = self._resolve_workspace_team_scope(
            _channel_id, self._event_team_id(event or {}, payload)
        )
        _claim_marker = (
            self._workspace_message_marker(_team_id or "", _ts)
            if _ts and _team_id is not None
            else None
        )
        # getattr: bare test doubles (object.__new__) may lack the map.
        _claims = getattr(self, "_processed_message_ts", None)
        _was_claimed = (
            _claim_marker is not None
            and _claims is not None
            and _claim_marker in _claims
        )
        try:
            ...
            if (
                ...
                logger.warning(
                    "[%s] handler failed after claiming team=%s ts=%s; claim released "
    ...
    async def _prefilter_inbound(
        ...
        """Normalize and admit one inbound message before any expensive hydration."""
        if logger.isEnabledFor(logging.DEBUG):
            ...
            logger.debug(
                ...
                event.get("user", "") or "",
                event.get("bot_id", "") or "",
                (bot_profile.get("name") if isinstance(bot_profile, dict) else "") or "",
                event.get("channel", ""),
                event.get("ts", ""),
                event.get("thread_ts", ""),
            )
        message_changed = event.get("subtype") == "message_changed"
        if message_changed:
        ...
        if message_changed:
            original_message_ts = str(event.get("ts") or "")
            marker = self._workspace_message_marker(team_id, original_message_ts)
            if original_message_ts and marker in self._processed_message_ts:
                return None

        # Rejected deliveries do not consume a dedup identifier needed by a later
        # admitted redelivery.
        event_ts = event.get("_slack_changed_event_ts") or event.get("ts", "")
        if event_ts and self._dedup.is_duplicate(self._workspace_event_id(team_id, event_ts)):
            return None
        ...
        if not sender_is_bot and msg_user and not event.get("client_msg_id"):
            sender_is_bot = await self._resolve_user_is_bot(
                msg_user, chat_id=channel_id, team_id=team_id
            )
        if sender_is_bot:
            bot_uid = self._bot_user_id_for_team_scope(team_id)
            own_bot_id = self._bot_id_for_team_scope(team_id)
            event_bot_id = str(event.get("bot_id") or "")
            if not bot_uid or (not msg_user and not own_bot_id):
                logger.debug("[Slack] Dropping bot message with unresolved workspace identity")
                return None
            allow_bots = self._slack_allow_bots()
            if allow_bots == "none":
                return None
            if allow_bots == "mentions" and bot_uid not in slack_user_mentions(
                _slack_mention_detection_text(event)
            ):
                return None
            if (msg_user and msg_user == bot_uid) or (
                event_bot_id and event_bot_id == own_bot_id
            ):
                return None

        if event.get("subtype") == "message_deleted":
            return None
        return event, team_id, channel_id
    ...
    def _apply_bot_mention(
        ...
        """Strip this bot's mention, normalize a hidden command, and remember the thread."""
        ...
        if (
            ...
            self._register_mentioned_thread(
                thread_ts, team_id=team_id, channel_id=channel_id
            )
    ...
    async def _handle_slack_message_impl(self, event: dict, payload: Optional[dict] = None) -> None:
        ...
        if not is_one_to_one_dm and not bot_uid:
            logger.debug("[Slack] Dropping channel message with unresolved workspace scope")
            return
        # Mentions may live only in Block Kit blocks. Parse both bare and pipe-form
        # Slack user mentions, and isolate configured regex evaluation off-loop.
        routing_text = _slack_mention_detection_text(event) or original_text or ""
        slack_bot_mention = bool(
            bot_uid and bot_uid in slack_user_mentions(routing_text)
        )
        pattern_bot_mention = False
        if not slack_bot_mention:
            pattern_bot_mention = await asyncio.to_thread(
                self._slack_message_matches_mention_patterns, routing_text
            )
        ...
        # Internal triggers skip the mention requirement but not authorization,
        # workspace admission, channel allowlists, or routing policy.
        ...
        if not is_one_to_one_dm:
            ...
            sender_is_peer_bot = self._event_declares_bot_sender(event)
            if not sender_is_peer_bot and user_id and user_id != bot_uid:
                sender_is_peer_bot = await self._resolve_user_is_bot(
                    user_id, chat_id=channel_id, team_id=team_id
                )
        ...
        if _claim_ts:
            self._remember_processed_message_ts(_claim_ts, team_id)
        if is_mentioned:
        ...
        if context_channel_id and context_channel_id != channel_id and not is_command_text:
            msg_event.text = (
                ...
                f"{msg_event.text}")
        if ts:
            self._remember_processed_message_ts(ts, team_id)
    ...
    async def _post_interactive_blocks(
        ...
        if thread_ts:
            kwargs["thread_ts"] = thread_ts
        # ``team_scoped`` controls the resolved-state key shape, not client
        # selection: explicit outbound workspace metadata is always authoritative.
        team_id = self._metadata_team_id(metadata)
    ...
    async def _begin_interaction(
        ...
        if team_id is None:
            logger.debug(
                "[Slack] Dropping %s interaction with unresolved workspace client",
                kind,
            )
            return None
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        if team_id is None:
            logger.debug(
                "[Slack] Dropping slash command with unresolved workspace client"
            )
            return
        if team_id and channel_id:
            self._remember_channel_team(channel_id, team_id)
...
def _apply_yaml_config(yaml_cfg: dict, slack_cfg: dict) -> dict | None:
    """Bridge Slack-owned YAML settings into environment and ``PlatformConfig.extra``.

    Environment variables retain precedence. Structured routing policy remains
    plugin-owned and is returned for the generic config loader to seed into
    ``extra``.
    """
    ...
    thread_routing = slack_cfg.get("thread_routing")
    if isinstance(thread_routing, dict):
        return {"thread_routing": thread_routing}
    return None
