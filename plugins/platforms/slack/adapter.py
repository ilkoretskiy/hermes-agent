...
def _slack_file_marker(file_obj: Dict[str, Any]) -> str:
    ...
    """
    name = str(
        file_obj.get("name") or file_obj.get("title") or file_obj.get("id") or "file"
    )
    name = re.sub(r"[\r\n\[\]]+", " ", name).strip() or "file"
...
class SlackAdapter(BasePlatformAdapter):
    ...
    # Native Slack streams create the user-visible history message; the
    # turn-final seals that same message rather than posting a second one.
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._team_bot_ids: Dict[str, str] = {}  # team_id → Slack bot_id
        ...
        self._mentioned_threads: set[Any] = set()
        self._MENTIONED_THREADS_MAX = 5000
        # Policy owns config parsing, validated control matching, and durable
        # strict state. The adapter retains only ephemeral routing evidence.
        self._thread_routing_policy = ThreadRoutingPolicy(
            self.config.extra.get("thread_routing")
        )
        ...
        # One active stream per turn, keyed by workspace/chat/message identity.
        # Each value: {"chat_id": str, "team_id": str, "ts": str,
        #  "draft_id": int, "sent": str, "started": float,
        #  "stopped": bool (present only while a sealed rewrite awaits retry)}
        ...
        # Per-turn reservations make chat.startStream atomic across concurrent
        # first frames without serializing independent Slack turns.
        self._native_stream_starts: Dict[str, asyncio.Future[None]] = {}
    ...
    @classmethod
    def _discard_oldest_slack_timestamps(cls, timestamps: set[str], count: int) -> None:
    ...
    def _trim_mentioned_threads(self) -> None:
        ...
        self._discard_oldest_by_thread_ts(
            ...
            lambda marker: marker[-1] if isinstance(marker, tuple) else marker,
        )
    ...
    def _resolve_workspace_team_scope(
        self, channel_id: str, team_id: str = ""
    ) -> Optional[str]:
        """Resolve an admitted workspace without borrowing another team's state.

        ``""`` represents a legacy unscoped runtime and is returned only when
        every scoped client and bot-identity registry is empty. ``None`` means
        scoped runtime state exists but does not identify one exact workspace.
        """
        ...
        channel_teams = getattr(self, "_channel_teams", {}).get(channel_id, set())
        if len(channel_teams) > 1:
            return None

        cached_team_id = getattr(self, "_channel_team", {}).get(channel_id, "")
        if cached_team_id:
            return str(cached_team_id)
        if len(channel_teams) == 1:
            return str(next(iter(channel_teams)))
        ...
        for attr in (
            ...
            registered_team_ids.update(getattr(self, attr, {}))
        if len(registered_team_ids) == 1:
            return str(next(iter(registered_team_ids)))
        if registered_team_ids:
            return None
        return ""

    def _admit_workspace_team_scope(
        self, channel_id: str, team_id: str = ""
    ) -> Optional[str]:
        """Resolve workspace scope and prove its exact client when scoped."""
        resolved_team_id = self._resolve_workspace_team_scope(channel_id, team_id)
        if resolved_team_id is None:
            return None
        if (
            resolved_team_id
            and self._get_client(channel_id, team_id=resolved_team_id) is None
        ):
            return None
        return resolved_team_id

    def _bot_user_id_for_team_scope(self, team_id: str) -> Optional[str]:
        """Resolve our bot user ID without crossing an ambiguous workspace scope."""
        team_bot_user_ids = getattr(self, "_team_bot_user_ids", {})
        if team_id:
            scoped_bot_user_id = team_bot_user_ids.get(team_id)
            if scoped_bot_user_id:
                return scoped_bot_user_id
            return self._bot_user_id if not team_bot_user_ids else None
        if len(team_bot_user_ids) == 1:
            return next(iter(team_bot_user_ids.values())) or self._bot_user_id
        if not team_bot_user_ids:
            return self._bot_user_id
        return None

    def _bot_id_for_team_scope(self, team_id: str) -> Optional[str]:
        """Resolve our Slack bot_id without crossing an ambiguous workspace scope."""
        team_bot_ids = getattr(self, "_team_bot_ids", {})
        if team_id:
            return team_bot_ids.get(team_id)
        if len(team_bot_ids) == 1:
            return next(iter(team_bot_ids.values())) or None
        return None
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # / ``_team_bot_user_ids`` / ``_team_bot_ids`` entries from the
            # prior session.
            ...
            async def handle_reaction_added(event, say, body):
                await self._handle_slack_reaction(event, body)

            @self._app.event("reaction_removed")
            async def handle_reaction_removed(event, say, body):
                await self._handle_slack_reaction(event, body, removed=True)
    ...
    async def disconnect(self) -> None:
        ...
        for stream in list(self._active_streams.values()):
            await self._seal_stream(stream["chat_id"], stream)
    ...
    @staticmethod
    def _workspace_thread_marker(team_id: str, channel_id: str, thread_ts: str) -> Any:
        """Scope ephemeral thread routing evidence by Slack-local dimensions."""
        if team_id:
    ...
    def _get_client(self, chat_id: str, team_id: Optional[str] = None) -> Any:
        ...
        if team_id:
            if team_id in team_clients:
                return team_clients[team_id]
            # An explicit scope must never borrow another workspace's client.
            # Preserve the legacy single-workspace path only when no scoped
            # workspace registry has been populated at all. During reconnect,
            # identity registries can outlive a temporarily empty client map;
            # that is still scoped runtime state, not an unscoped legacy path.
            scoped_registries = (
                team_clients,
                getattr(self, "_team_bot_user_ids", {}),
                getattr(self, "_team_bot_ids", {}),
                getattr(self, "_team_bot_names", {}),
            )
            if any(scoped_registries):
                return None
            app = getattr(self, "_app", None)
            return app.client if app else None
        cached_team_id = getattr(self, "_channel_team", {}).get(chat_id)
        if cached_team_id and cached_team_id in team_clients:
            return team_clients[cached_team_id]
        app = getattr(self, "_app", None)
        return app.client if app else None  # fallback to primary
    ...
    async def delete_message(self, chat_id: str, message_id: str) -> bool:
        ...
        if not self._app:
            return False
        try:
            response = await self._get_client(chat_id).chat_delete(
    ...
    def _native_stream_key(
        ...
        """Return a workspace/chat/turn key for a native Slack stream."""
        md = metadata or {}
        resolved_team_id = (
            team_id
            if team_id is not None
            else self._metadata_team_id(metadata)
            or self._channel_team.get(chat_id, "")
        )
        turn_id = md.get("message_id") or md.get("reply_to_message_id")
        if turn_id:
            return f"{resolved_team_id}:{chat_id}:turn:{turn_id}"
        anchor = md.get("thread_ts") or md.get("thread_id") or ""
        return f"{resolved_team_id}:{chat_id}:thread:{anchor}"

    def _match_active_stream_key(
        self,
        chat_id: str,
        metadata: Optional[Dict[str, Any]],
    ) -> Optional[str]:
        """Resolve the open stream owned by this turn-final send."""
        candidates = self._matching_active_stream_keys(chat_id, metadata)
        return candidates[0] if len(candidates) == 1 else None

    def _matching_active_stream_keys(
        self,
        chat_id: str,
        metadata: Optional[Dict[str, Any]],
    ) -> list[str]:
        """Find streams matching explicit scope or a unique turn identity."""
        md = metadata or {}
        team_id = self._metadata_team_id(metadata)
        turn_id = str(md.get("message_id") or md.get("reply_to_message_id") or "")
        anchor = str(md.get("thread_ts") or md.get("thread_id") or "")
        candidates = []
        for key, stream in self._active_streams.items():
            if str(stream.get("chat_id") or "") != str(chat_id):
                continue
            if team_id and str(stream.get("team_id") or "") != team_id:
                continue
            if turn_id and str(stream.get("turn_id") or "") != turn_id:
                continue
            if not turn_id and anchor and str(stream.get("thread_anchor") or "") != anchor:
                continue
    ...
    async def send_draft(
        ...
        if len(candidates) > 1:
            return SendResult(
                success=False,
                error="ambiguous native stream scope",
            )

        if candidates:
            stream_key = candidates[0]
            stream = self._active_streams[stream_key]
            # The workspace persisted with the visible Slack message is the
            # only authority for continuation. Do not admit/cache-resolve a
            # new workspace before recovering this existing stream.
            team_id = str(stream.get("team_id") or "")
            client = self._get_client(chat_id, team_id=team_id)
        else:
            # New streams must establish an unambiguous workspace before any
            # provider call. This admission does not apply to recovery of an
            # existing persisted stream above.
            team_id = self._admit_workspace_team_scope(
                chat_id,
            ...
            if team_id is None:
                return SendResult(
                    success=False,
                    error="ambiguous native stream workspace",
                )
            stream_key = self._native_stream_key(chat_id, metadata, team_id=team_id)
            stream = None
            client = self._get_client(chat_id, team_id=team_id)
        ...
        if client is None:
            return SendResult(
                success=False,
                error="native stream workspace client unavailable",
            )
        ...
        try:
            if stream is not None and stream.get("draft_id") != draft_id:
                ...
                if not sealed:
                    return SendResult(
                        success=False,
                        message_id=stream["ts"],
                        error="native stream segment finalization unconfirmed",
                        retryable=True,
                    )
                self._active_streams.pop(stream_key, None)
                stream = None

            pending_start = self._native_stream_starts.get(stream_key)
            if stream is None and pending_start is not None:
            ...
            if stream is None:
                ...
                start_complete = asyncio.get_running_loop().create_future()
                self._native_stream_starts[stream_key] = start_complete
                try:
                    response = await client.chat_startStream(**start_kwargs)
                finally:
                    if not start_complete.done():
                        start_complete.set_result(None)
                    self._native_stream_starts.pop(stream_key, None)
                ...
                self._active_streams[stream_key] = {
                    ...
                    "thread_anchor": str(
                        (metadata or {}).get("thread_ts")
                        or (metadata or {}).get("thread_id")
                        or ""
                    ),
                    "ts": str(ts),
                    "draft_id": draft_id,
            ...
            # Append path: the persisted stream workspace owns every later
            # mutation, even if continuation metadata lost scope or the
            # chat-only cache drifted after startStream.
            ...
            if stream.get("append_uncertain"):
                # The prior append may have reached Slack despite a lost
                # acknowledgement. Never replay a delta whose visibility is
                # unknown; only authoritative same-ts finalization can safely
                # reconcile this stream.
                return SendResult(
                    success=False,
                    message_id=stream["ts"],
                    error="native stream append state unconfirmed",
                    retryable=True,
                )
            if not text.startswith(sent):
                ...
                if not sealed:
                    return SendResult(
                        success=False,
                        message_id=stream["ts"],
                        error="native stream prefix finalization unconfirmed",
                        retryable=True,
                    )
                self._active_streams.pop(stream_key, None)
                return SendResult(
            ...
            delta = text[len(sent):]
            try:
                await stream_client.chat_appendStream(
                    channel=chat_id,
                    ts=stream["ts"],
                    markdown_text=delta,
                )
            ...
            # Once startStream has created the visible Slack message, any
            # append failure is ambiguous. Retain ownership of its ts so the
            # authoritative final can stop/update that same message instead
            # of falling through to a duplicate chat.postMessage.
            owned_stream = self._active_streams.get(stream_key)
            ...
            return SendResult(
                success=False,
                message_id=(owned_stream or {}).get("ts"),
                error=err,
                retryable=owned_stream is not None,
            )
    ...
    async def abandon_open_draft(
        ...
        """Best-effort seal and disarm for a cancelled/stale native turn."""
        stream_key = self._match_active_stream_key(chat_id, metadata)
        ...
        if stream is None:
            return
        try:
            await self._seal_stream(
                chat_id,
                stream,
                final_text=self._strip_stream_cursor(content),
            )
        finally:
            # Abandoned turns have no later authoritative retry owner. Never
            # leave interception state armed for a subsequent turn.
            self._active_streams.pop(stream_key, None)
            await self.stop_typing(chat_id, metadata=metadata)
    ...
    async def _seal_stream(
        ...
        try:
            ...
            if final_text is not None and not stream.get("append_uncertain"):
                sent = stream.get("sent", "")
    ...
    async def _try_finalize_stream(
        ...
        if stream is None:
            return None
        sent = stream.get("sent", "")
        append_uncertain = bool(stream.get("append_uncertain"))
        text = self._strip_stream_cursor(content)
        # A regular send can only claim the stream by extending its visible
        # text. The stream consumer marks its authoritative turn-final with
        # ``notify``; that payload may be a non-prefix normalization of the
        # streamed preview, and must still seal/update the existing stream
        # rather than create a second Slack message. Interim sends have no
        # marker and retain the normal post path.
        extends_stream = bool(sent) and text.startswith(sent)
        authoritative_final = bool((metadata or {}).get("notify"))
        if not extends_stream and not authoritative_final:
            return None
        ts = stream["ts"]
        ok = await self._seal_stream(chat_id, stream, final_text=text)
        if not ok:
            # A transport failure is ambiguous: Slack may have stopped the
            # stream even when the acknowledgement was lost. Keep ownership
            # of the original ts and report a retryable failure instead of
            # posting a second message.
            return SendResult(
                success=False,
                message_id=ts,
                error="native stream finalization unconfirmed",
                retryable=True,
            )
        stream["stopped"] = True
        # A non-prefix authoritative final cannot be supplied to stopStream's
        # append-only markdown argument. Update the sealed stream message in
        # place instead, preserving its ts and applying the same final Block
        # Kit layout as the edit finalization path.
        blocks = self._maybe_blocks(text)
        if blocks or not extends_stream or append_uncertain:
            try:
                update_kwargs: Dict[str, Any] = {
                    "channel": chat_id,
                    "ts": ts,
                ...
                if not extends_stream or append_uncertain:
                    # The preview stream is sealed, but the authoritative
                    # rewrite is not visible yet. Retain the stopped stream so
                    # a retry updates this same ts without another stop/post.
                    return SendResult(
                        success=False,
                        message_id=ts,
                        error=str(e),
    ...
    async def send_typing(self, chat_id: str, metadata=None) -> None:
        ...
        if self._is_ignored_channel(chat_id):
            logger.debug(
                "[Slack] Suppressed typing/status in configured ignored channel %s",
                chat_id,
            )
            return
        if not self._app:
    ...
    async def stop_typing(self, chat_id: str, metadata=None) -> None:
        ...
        if self._is_ignored_channel(chat_id):
            logger.debug(
                "[Slack] Suppressed status clear in configured ignored channel %s",
                chat_id,
            )
    ...
    async def _humanize_user_mentions(
        ...
        if not text or "<@" not in text:
            return text
        mention_tokens = slack_user_mention_tokens(text)
        if not mention_tokens:
            return text

        display_names: dict[str, str] = {}
        for uid, _, _ in mention_tokens:
            ...
            display_names[uid] = (name or uid).strip() or uid

        parts: list[str] = []
        cursor = 0
        for uid, start, end in mention_tokens:
            parts.append(text[cursor:start])
            parts.append(f"@{display_names[uid]}")
            cursor = end
    ...
    async def _handle_assistant_thread_lifecycle_event(
        ...
        channel_id = metadata.get("channel_id", "")
        team_id = self._admit_workspace_team_scope(
            channel_id, metadata.get("team_id", "")
        )
        if team_id is None:
            logger.debug(
                "[Slack] Dropping Assistant lifecycle event with unresolved "
                "workspace scope"
            )
            return
    ...
    async def _handle_app_context_changed(
        ...
        team_id = self._admit_workspace_team_scope(
            context_channel_id, self._event_team_id(event, body)
        )
        if team_id is None:
            logger.debug(
                "[Slack] Dropping app_context_changed with unresolved workspace scope"
            )
            return
    ...
    async def _handle_app_home_opened(
        ...
        user_id = event.get("user") or event.get("user_id") or ""
        team_id = self._admit_workspace_team_scope(
            channel_id, self._event_team_id(event, body)
        )
        if team_id is None:
            logger.debug(
                "[Slack] Dropping app_home_opened with unresolved workspace scope"
            )
            return
        context_channel_id = self._context_channel_id(context)
    ...
    async def _handle_slack_reaction(
        ...
        if team_id is None:
            logger.debug("[Slack] Dropping reaction with unresolved workspace scope")
            return
        bot_uid = self._bot_user_id_for_team_scope(team_id)
        if not bot_uid:
            logger.debug("[Slack] Dropping reaction with unresolved workspace scope")
            return
        ...
        if client is None:
            logger.debug("[Slack] Dropping reaction with unresolved workspace client")
            return
        ...
        # message's ts).
        synthetic_ts = (
            event.get("event_ts") or f"reaction-{msg_ts}-{reaction_name}-{user_id}"
        )
        synthetic: dict = {
            "type": "message",
            "user": user_id,
    ...
    def _slack_reaction_triggers(self) -> Optional[set]:
        ...
        if isinstance(raw, (list, tuple, set)):
            names = {
                str(p).strip().strip(":") for p in raw if str(p).strip().strip(":")
            }
            return names or set()
    ...
    async def _handle_slack_file_shared(
        ...
        if self._is_ignored_channel(channel_id):
            logger.info(
                "[Slack] Ignoring file_shared event in configured ignored channel %s",
                channel_id,
            )
            return
        ...
        if team_id is None:
            logger.debug(
                "[Slack] Dropping file_shared event with unresolved workspace client"
            )
            return
    ...
    def _register_mentioned_thread(
        self, thread_ts: str, team_id: str = "", channel_id: str = ""
    ) -> None:
        """Record a thread as bot-mentioned so future replies auto-trigger."""
        if not thread_ts or (team_id and not channel_id):
            return
        self._mentioned_threads.add(
            self._workspace_thread_marker(team_id, channel_id, thread_ts)
        )
    ...
    async def _should_wake_on_unmentioned_message(
        ...
        if not event_thread_ts:
            return False
        bot_message_marker = self._workspace_message_marker(team_id, event_thread_ts)
        mentioned_thread_marker = self._workspace_thread_marker(
            team_id, channel_id, event_thread_ts
        )
        if is_thread_reply and bot_message_marker in self._bot_message_ts:
            return True
        if mentioned_thread_marker in self._mentioned_threads:
            return True
        ...
        if is_thread_reply:
            ...
            if bot_uid:
                ...
                if parent_text and bot_uid in slack_user_mentions(parent_text):
    ...
    async def _handle_slack_message_impl(
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            _bot_name = (
                _bot_profile.get("name") if isinstance(_bot_profile, dict) else ""
            ) or ""
            logger.debug(
        ...
        if self._is_ignored_channel(channel_id):
            logger.info(
                "[Slack] Ignoring message in configured ignored channel %s", channel_id
            )
            return
        ...
        event_team_id = self._admit_workspace_team_scope(
            channel_id, self._event_team_id(event, payload)
        )
        if event_team_id is None:
            logger.debug("[Slack] Dropping message with unresolved workspace scope")
            return

        # Dedup only after workspace admission. Rejected deliveries must not
        # consume the identifier that a later admitted redelivery needs.
        event_ts = event.get("_slack_changed_event_ts") or event.get("ts", "")
        if event_ts and self._dedup.is_duplicate(
            self._workspace_event_id(event_team_id, event_ts)
        ):
            return
        event_bot_uid = self._bot_user_id_for_team_scope(event_team_id)
        event_bot_id = str(event.get("bot_id") or "")
        event_own_bot_id = self._bot_id_for_team_scope(event_team_id)
        ...
        if sender_is_bot:
            if not event_bot_uid:
                logger.debug(
                    "[Slack] Dropping bot message with unresolved workspace scope"
                )
                return
            if not msg_user and not event_own_bot_id:
                logger.debug(
                    "[Slack] Dropping userless bot message with unresolved bot identity"
                )
                return
            ...
            if allow_bots == "none":
                ...
                if not event_bot_uid or event_bot_uid not in slack_user_mentions(
                    text_check
                ):
                    logger.debug(
                        "[Slack] Dropping bot message under allow_bots=mentions: "
                        "no current-workspace bot mention in flat text or blocks"
                    )
                    return
            # "all" falls through to process the message
            # Always ignore our own messages to prevent echo loops. Slack bot
            # events may omit ``user``; ``bot_id`` from auth.test is the
            # workspace-scoped identity for that event shape.
            if (msg_user and msg_user == event_bot_uid) or (
                event_bot_id and event_bot_id == event_own_bot_id
            ):
                return
        ...
        if blocks and not is_command_text:
            blocks_text = _extract_additional_text_from_slack_blocks(
                ...
                bot_uid=self._team_bot_user_ids.get(
                    event_team_id, self._bot_user_id
                )
                or "",
            )
            if blocks_text:
        ...
        channel_id = event.get("channel", "")
        ts = event.get("ts", "")
        # Keep the scope resolved by the pre-admission gate authoritative. It
        # may come from a unique registered workspace when Slack omits team_id;
        # re-deriving from the raw event here would lose that scope and let
        # downstream user/client/session work fall back to the primary workspace.
        outer_team_id = event_team_id
        ...
        bot_uid = self._bot_user_id_for_team_scope(str(team_id or ""))
        if not is_one_to_one_dm and not bot_uid:
            logger.debug(
                "[Slack] Dropping channel message with unresolved workspace scope"
            )
            return
        ...
        slack_bot_mention = bool(
            bot_uid and self._slack_message_mentions_self(routing_text, {bot_uid})
        )
        pattern_bot_mention = False
        if not slack_bot_mention:
            pattern_bot_mention = await asyncio.to_thread(
                self._slack_message_matches_mention_patterns,
                routing_text,
            )
        is_mentioned = slack_bot_mention or pattern_bot_mention
        mention_command_text = ""
        if is_mentioned and bot_uid:
            mention_stripped_probe = strip_slack_user_mentions(
                original_text, {bot_uid}
            ).strip()
            if mention_stripped_probe.startswith("/"):
                mention_command_text = mention_stripped_probe
            else:
                rewritten_probe = _rewrite_known_bang_command(mention_stripped_probe)
                if rewritten_probe.startswith("/"):
                    mention_command_text = rewritten_probe
        ...
        if not is_one_to_one_dm:
            ...
            # Apply the isolated state/policy decision before thread context or
            # files are hydrated. The adapter supplies only Slack routing facts;
            # durable state and control semantics belong to ThreadRoutingPolicy.
            control_thread_ts = thread_ts or event_thread_ts or ts
            has_thread_session = False
            if self._thread_routing_policy.enabled and control_thread_ts:
            ...
            direct_bot_mention = is_mentioned
            policy_decision = await asyncio.to_thread(
                self._thread_routing_policy.evaluate,
                RoutingContext(
                    team_id=team_id,
                    channel_id=channel_id,
                    thread_ts=control_thread_ts,
                    ...
                    message_ts=ts,
                    text=routing_text,
                    direct_bot_mention=direct_bot_mention,
                    active_agent_thread=has_scoped_active_thread,
                    mentions_other_user=ThreadRoutingPolicy.mentions_other_user(
                        routing_text, bot_uid
                    ),
                    is_command=bool(is_command_text or mention_command_text),
                    peer_bot=sender_is_bot_user,
                ),
            )
            if policy_decision.disposition in {
            ...
            elif channel_id not in self._slack_require_mention_channels() and (
        ...
        if is_mentioned:
            ...
            bot_user_ids = {bot_uid} if bot_uid else set()
            text = strip_slack_user_mentions(text, bot_user_ids).strip()
            ...
            # at the bot).
            mention_stripped = strip_slack_user_mentions(
                original_text, bot_user_ids
            ).strip()
            if mention_command_text:
                command_text = mention_command_text
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        team_id = self._admit_workspace_team_scope(
            channel_id, self._event_team_id({}, body)
        )
        if team_id is None:
            logger.debug(
                "[Slack] Dropping slash-confirm action with unresolved workspace scope"
            )
            return
        client = self._get_client(channel_id, team_id=team_id or None)
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        if team_id is None:
            logger.debug(
                "[Slack] Dropping approval action with unresolved workspace scope"
            )
            return
        client = self._get_client(channel_id, team_id=team_id or None)
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if team_id is None:
            logger.debug(
                "[Slack] Dropping clarify action with unresolved workspace scope"
            )
            return
        client = self._get_client(channel_id, team_id=team_id or None)
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        team_id = self._admit_workspace_team_scope(
            channel_id, command.get("team_id", "")
        )
        if team_id is None:
            logger.debug(
                "[Slack] Dropping slash command with unresolved workspace scope"
            )
            return
        # Track which workspace owns this channel
        if team_id and channel_id:
    ...
    def _slack_thread_routing_enabled(self) -> bool:
        return self._thread_routing_policy.enabled

    @staticmethod
    def _slack_thread_routing_key(team_id: str, channel_id: str, thread_ts: str) -> str:
        return ThreadRoutingPolicy.key(team_id, channel_id, thread_ts)
    ...
    def _mark_slack_thread_active(
        ...
        if len(self._slack_active_thread_keys) > self._SLACK_ACTIVE_THREAD_KEYS_MAX:
            excess = (
                len(self._slack_active_thread_keys)
                - self._SLACK_ACTIVE_THREAD_KEYS_MAX // 2
            )
    ...
    def _slack_message_addressed_to_other_user(self, text: str, self_uids: set) -> bool:
        ...
        if close < 0:
            return False
        mentioned = slack_user_mentions(stripped[: close + 1])
        return len(mentioned) == 1 and mentioned.isdisjoint(self_uids)
    ...
    def _slack_message_mentions_self(self, text: str, self_uids: set) -> bool:
        ...
        pipe-form bot mention as "also mentioned". It shares the bounded parser
        used by bot-event and thread-routing gates so malformed markup cannot be
        classified differently at separate stages.
        """
        return not slack_user_mentions(text).isdisjoint(self_uids)
    ...
    def _slack_mention_patterns(self) -> tuple[tuple[str, "re.Pattern[str]"], ...]:
        ...
        compiled = compile_bounded_patterns(patterns, kind="mention")
        if compiled:
    ...
    def _slack_message_matches_mention_patterns(self, text: str) -> bool:
        ...
        if not text:
            return False
        return match_bounded_pattern(self._slack_mention_patterns(), text) is not None
...
def _apply_yaml_config(yaml_cfg: dict, slack_cfg: dict) -> dict | None:
    ...
    if "ignore_other_user_mentions" in slack_cfg and not os.getenv(
