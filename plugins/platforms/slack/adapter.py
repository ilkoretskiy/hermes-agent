    ...
    async def send(
        ...
        try:
            ...
            if (
                ...
                # blocks. Only applied to a single-chunk message — a >39k
                # response that had to be split is pathological for Block Kit's
                # 50-block / 3000-char limits, so those fall back to plain text.
                # The ``text`` field is always kept as the notification /
                # accessibility fallback. The production markdown-block rollout
                # above intentionally wins when enabled for a channel.
        ...
        *,
        ...
        try:
            ...
            if (
                ...
                update_kwargs["blocks"] = [{"type": "markdown", "text": content}]
            elif finalize:
                # Only render Block Kit on the FINAL edit. Intermediate
                # streaming edits stay plain mrkdwn — re-deriving a full block
                # layout on every progressive flush would be wasteful and
                # jittery. ``text`` is kept as the fallback either way.
            ...
            try:
                ...
                if "blocks" not in update_kwargs:
                    raise
                logger.warning(
                    "[Slack] Block update failed; falling back to legacy text update: %s",
                    e,
                    exc_info=True,
                )
