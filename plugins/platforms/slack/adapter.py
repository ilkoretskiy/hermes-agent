...
def _apply_yaml_config(yaml_cfg: dict, slack_cfg: dict) -> dict | None:
    ...
    under a multiplexed secondary profile's scope) + ``PlatformConfig.extra``. Structured
    Planner5D thread-routing policy remains plugin-owned and is copied to ``extra`` without an
    environment-variable compatibility shim.
    """
    extra = _apply_yaml_bridge(slack_cfg, _YAML_BRIDGE) or {}
    thread_routing = slack_cfg.get("thread_routing")
    if isinstance(thread_routing, dict):
        extra["thread_routing"] = thread_routing
