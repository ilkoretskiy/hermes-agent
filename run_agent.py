...
_EPHEMERAL_SCAFFOLDING_FLAGS = (
    ...
    # Codex-ack continuation nudge + its synthetic interim assistant turn
    # (conversation_loop.py). They exist only for the immediate retry API call
    # and must never reach durable history — a resumed session would otherwise
    # see a bare ``developer``-role row with no way to filter it.
    "_codex_ack_continuation_synthetic",
    "_codex_ack_continuation_interim",
)
    ...
    def _flush_messages_to_session_db(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        try:
            ...
            for _msg_idx, msg in enumerate(messages):
                ...
                # (Codex-ack continuation items are covered too — see
                # _EPHEMERAL_SCAFFOLDING_FLAGS.)
